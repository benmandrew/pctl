module Make (Ma : Modal.MODAL) (Me : Model.MODEL) (F : Formula.FORMULA) = struct
  module F = F (Me.Aprop)
  module Ma = Ma (Me) (F)
  ...
  let label_init states =
    let init_props s =
      Me.Aprop.Set.fold
        (fun p f -> F.Set.add (F.Prop p) f)
        s.Me.State.l F.Set.empty
    in
  ...
  let add_f_to_state labels i f =
    let s = F.Set.add f (Int_map.find i labels) in
  ...
  let merge_labels _ s s' =
    ...
    | None, None -> None
    | s, s' ->
        let s = Option.value s ~default:F.Set.empty in
        let s' = Option.value s' ~default:F.Set.empty in
        Some (F.Set.union s s')

  module Label = struct
    let rec v_neg states labels f =
      let labels = v_state states labels f in
      fun i -> not (F.f_in_state_labels labels i f)
    ...
    and v_forall states labels f = v_path states labels ~p:1.0 ~op:F.Geq f
    and v_exists states labels f = v_path states labels ~p:0.0 ~op:F.Gt f
    ...
    and v_strong_until states labels ~t ~p ~op f f' =
      let labels =
        Int_map.merge merge_labels (v_state states labels f)
          (v_state states labels f')
      in
      let b = Ma.strong_until states labels ~t ~p ~op f f' in
      fun i -> b.(i)

    and v_weak_until states labels ~t ~p ~op f f' =
      let p = 1.0 -. p in
      let op = match op with F.Geq -> F.Gt | Gt -> Geq in
      fun i ->
        not
    ...
    and v_generally states labels ~t ~p ~op f =
      v_weak_until states labels ~t ~p ~op f (F.Bool false)

    and v_finally states labels ~t ~p ~op f =
      v_strong_until states labels ~t ~p ~op (F.Bool true) f

    and v_leads_to states labels ~t ~p ~op f f' =
      let f =
        F.(Generally (Infinity, Impl (f, P (op, p, Finally (t, f')))))
      in
    ...
    and v_state states labels f =
      let pred labels =
        ...
        | A f -> v_path states labels ~p:1.0 ~op:F.Geq f
        | E f -> v_path states labels ~p:0.0 ~op:F.Gt f
  ...
  let v k f =
    let states = k.Me.Kripke.states in
    let starting_labels = label_init states in
    let labels = Label.v states starting_labels f in
    let labels_for_state = Int_map.find k.Me.Kripke.initial labels in
    F.Set.mem f labels_for_state
end